...
describe('SettingsPage', () => {
  ...
  it('renders notification card with toggle', () => {
    render(<SettingsPage />);
  ...
  it('has notification action buttons', () => {
    ...
    expect(screen.getAllByText('시스템 설정').length).toBeGreaterThanOrEqual(1);
  });

  it('does not render removed sections', () => {
    render(<SettingsPage />);
    expect(screen.queryByText('운영 정책')).toBeNull();
    expect(screen.queryByText('시스템 안내')).toBeNull();
  });
