...
1. [x] 비로그인 상태 `/today` 진입 -> `/auth?next=/today`로 이동
2. [x] 카카오 로그인 성공 -> `/today` 자동 복귀
3. [x] 비로그인 상태 `/friends` 진입 -> 로그인 후 `/friends` 복귀
4. [x] 카카오 로그인 취소 -> `/auth`에 머물며 에러 메시지 표시
5. [x] 앱 재실행 후 세션 유지 -> `/auth` 재진입 없이 보호 페이지 접근 가능

검증 메모 (2026-03-20):
- 취소/실패 분기 문구를 분리하고 `다시 시도하기` 액션 제공
- callback 후 세션 반영 지연 케이스를 위해 세션 재시도 + auth state listener 적용
- 친구 페이지 인증 선행 흐름(`/friends` -> `/auth?next=/friends` -> `/friends`) 재검증
